...
class SAC():
    def __init__(self, env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
                 ...
                 logger_kwargs=dict(), save_freq=1, use_kl_loss=False, epsilon=1e-5, target_KL_div=0,
                 target_entropy=0.3):
    ...
    def compute_loss_q(self, data):
        ...
        with torch.no_grad():
            ...
            KL_loss = compute_kld_univariate_gaussians(mu_prior2, torch.tensor(sigma_prior).to(self.device), mu_policy2,
    ...
    def compute_loss_pi(self, data):
        ...
        KL_loss = compute_kld_univariate_gaussians(mu_prior, torch.tensor(sigma_prior).to(self.device), mu_policy,
...
def run(agents, env):
    ...
    for t in range(NUM_STEPS):
        ...
        if t > agent.start_steps:
            ...
            if METHOD == "CORE-RL":
                with torch.no_grad():
                    act_b, _, _, _ = agent.ac.pi(torch.as_tensor(o_old, dtype=torch.float32).to(agent.device), True,
                                                 False)
                    base_q = agent.ac.q1(torch.as_tensor(o_old, dtype=torch.float32).to(agent.device),
                                         act_b).cpu().numpy()

                    act_t, _, _, _ = agent.ac.pi(torch.as_tensor(o, dtype=torch.float32).to(agent.device), True, False)
                    target_q = agent.ac.q1(torch.as_tensor(o, dtype=torch.float32).to(agent.device),
                                           act_t).cpu().numpy()
...
wandb.run.name = TASK + "_" + "SEED_" + str(SEED) + "_" + METHOD + "_" + time.asctime().replace(' ', '_').replace(":", "_")

ROOT_DIR = os.path.dirname(os.path.abspath(__file__))

save_dir = ROOT_DIR + "\\saved_models\\" + wandb.run.name
os.mkdir(save_dir)
...
max_ep_steps = 1000
env = ReacherEnv(headless=True, dense=True if REWARD == "dense" else False, max_episode_steps=max_ep_steps)
